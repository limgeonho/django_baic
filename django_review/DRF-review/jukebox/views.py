...
def article_list_or_create(request):
    ...
    def artist_list():
        ...
        serializer = AlbumListSerializer(artists, many=True)
...
def artist_detail_or_update_or_delete(request, artist_pk):
    ...
    def update_artist():
        serializer = ArtistListSerializer(instance=artist, data=request.data)
        if serializer.is_valid(raise_exception=True):
            serializer.save()
            return Response(serializer.data)
...
def album_detail_or_update_or_delete(request, album_pk):
    ...
    if request.method == 'GET':
        ...
        serializer = AlbumListSerializer(instance=album, data=request.data)
        if serializer.is_valid(raise_exception=True):
            serializer.save()
            return Response(serializer.data)
    elif request.method == 'DELETE':
...
def track_detail_or_update_or_delete(request, album_pk, track_pk):
    ...
    def update_track():
        serializers = TrackSerializer(instance=track, data=request.data)
        if serializers.is_valid(raise_exception=True):
            serializers.save(album=album)
